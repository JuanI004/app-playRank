...
export default function Playlist() {
  const { playlist, toggleInPlaylist } = usePlaylist();
    ...
    cantRateados > 0
      ? (
          playlist
            ...
            .reduce((prom, g) => prom + getRating(g.id), 0) / cantRateados
        ).toFixed(1)
      : "-";
  const metacriticProm = (
    playlist.reduce((prom, g) => prom + g.metacritic, 0) / playlist.length
  ).toFixed(0);

  function handleDelete(game) {
    toggleInPlaylist(game);
  }
      ...
      <main className="relative max-w-[1200px] mx-auto flex gap-10 flex-col p-4">
        <div className="flex w-full justify-between mt-10  ">
          ...
          <button
            ...
            className="font-pixel text-primary text-xs border border-primary py-4 w-50 cursor-pointer hover:bg-[#ffd90023] cursor pointer transition-colors"
